        ...
        if (Array.isArray(obj[key])) {
            ...
            for (var j = 0; j < l; j++) {
                ...
                if (typeof(elem) == 'object') {
                    self.openTag(key);
                    self.parseArrayObj(elem);
                    self.arrayCloseTag(key);
                }
            }
...
ToXml.prototype.parseArrayObj = function(obj) {
    var self = this;
    var keys = Object.keys(obj);
    var len = keys.length;
...
ToXml.prototype.arrayCloseTag = function(key) {
    this.arrayCompleteTag();
}
...
ToXml.prototype.arrayCompleteTag = function() {
    if (this.tagIncomplete) {
        this.xml += ' />';
        this.tagIncomplete = false;
    }
